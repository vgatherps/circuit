...
//     and VERY cheaply approximate e^x by converting to int and shifting updates
//     basically since you get the exponent you want in the mantissa, and remaining
//     bits linearly interpolate between the two. I think this one is more accurate
//     and will almost certainly be faster then the schraudolf version where you
//     compute two sides and do a division...
//     You can also do this approximation for any base, including two
...
// The ieee representation of a 32-bit floating point value is:
// * neeeeeeeeeeemmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm
// * one sign bit, n
// * 8 exponent bits, e
// * 23 mantissa bits, m
// which computes (-1)^n * (1.m23_m22_m21...m_1_m_0) * 2^(a - 127) [1.... is a binary fraction]
//
...
// If we consider another representation of multiplying the two values:
// 0(ax8)(mx23) for the middle value, 2^[0x_lower/0x100]
// (fx9)(0x23) for the plain power of two, 2^0x_exp
//
...
// We certainly want to make this work with an arbitrary base, as 2^1/0x100 is somewhat useless
// For some multiplier,
// base ^ exp == [2^1/0x100] ^ (multiplier * exp)
// setting multipler to 0x100 * ln(base) / log(2),
...
double do_compute(FastExpCache32 &a, float d)
{
    return a.compute(d);
}