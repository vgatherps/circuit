...
void TickCompletionDetector::schedule_invalidation_at(
    std::uint64_t invalidate_at, TimerHandle reschedule) {
...
void TickCompletionDetector::do_init(const nlohmann::json &params) {
  ...
  ns_till_batch_invalidation =
      params["ns_till_batch_invalidation"].get<double>();
}