...
class DecayingSum {
  ...
  double compute_and_schedule_decay(std::uint64_t now, double current_sum,
...
public:
  ...
  requires(HAS_OPT_REF(I, double, tick) &&
           ...
           HAS_REF_FIELD(O, double, running_sum)) void on_tick(I input,
                                                               O output) {
    if (!(input.tick.valid() && input.time.valid())) {
      return;
    }

    double decayed_sum = this->compute_decay(*input.time, output.running_sum);
  ...
  requires(HAS_OPT_REF(I, std::uint64_t, time) &&
           ...
           HAS_FIELD(M, TimerHandle, timer)) void decay(I input, O output,
                                                        M metadata) {
    if (input.time.valid()) {
      double decayed_sum = this->compute_and_schedule_decay(
          *input.time, output.running_sum, metadata.timer);
  ...
  requires(HAS_REF_FIELD(O, double, running_sum) &&
           HAS_FIELD(M, TimerHandle,
                     timer)) void init(O output, M metadata,
